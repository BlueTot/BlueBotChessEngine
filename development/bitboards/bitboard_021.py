...
spec = [
    ("from_square", nb.uint8),
    ("to_square", nb.uint8),
    ("en_passant", nb.boolean),
    ("promotion", nb.uint8),
    ("castling", nb.boolean)
]
...
spec = [
    ("__board_bb", nb.types.List(nb.uint64)),
    ("__turn", nb.boolean),
    ("__castling_rights", nb.types.List(nb.boolean)),
    ("__en_passant_square", nb.uint8),
    ("__move_stack", )
    ("__state_stack", nb.types.List(nb.types.Tuple(nb.types.List(nb.unit64)))),
]
...
class Board:
    ...
    def pseudo_legal_moves(self): # Pseudo-legal moves
        return self.generate_pawn_moves() + self.generate_knight_moves() + self.generate_bishop_moves() + \
               self.generate_rook_moves() + self.generate_queen_moves()
    ...
    def legal_moves(self): # Legal moves
        ...
        for move in pseudo_legal_moves:
            ...
            self.pop()
        return legal_moves + self.generate_king_moves() + self.generate_castling_moves()
...
v17
{'nodes': 14, 'captures': 1, 'checks': 2, 'en passant': 0, 'time': 0.015860199999999998}
{'nodes': 191, 'captures': 14, 'checks': 10, 'en passant': 0, 'time': 0.10432050000000001}
{'nodes': 2812, 'captures': 209, 'checks': 267, 'en passant': 2, 'time': 0.6769484}
{'nodes': 43238, 'captures': 3348, 'checks': 1680, 'en passant': 123, 'time': 2.2937762}
{'nodes': 674624, 'captures': 52051, 'checks': 52950, 'en passant': 1165, 'time': 23.4541634}
