...
class Board:
    ...
    PIECE_MAPPING = {(PAWN, WHITE): 0,
                     ...
                     (KING, WHITE): 10,
                     (KING, BLACK): 11}
    SCANS = [bsra, bsfa, bsfa, bsra, bsra, bsra, bsfa, bsfa]
    POSSIBLE_PINNERS = [(ROOK, QUEEN), (ROOK, QUEEN), (ROOK, QUEEN), (ROOK, QUEEN),
                        (BISHOP, QUEEN), (BISHOP, QUEEN), (BISHOP, QUEEN), (BISHOP, QUEEN)]
    ...
    def __white_pieces(self): # White pieces
        return reduce(lambda x, y : x | y, [self.__board_bb[i] for i in range(0, 12, 2)])

    def __black_pieces(self): # Black pieces
    ...
    @staticmethod
    def __squares_from(bb): # Get squares from bitboard
    ...
    #     king = sqr_from_bb(self.__board_bb[10 if self.__turn else 11])
    #     blockers = self.__blockers()
        
    #     n = (nB := ROOK_SQRS[king]["N"]) & blockers # North
    #     e = (eB := ROOK_SQRS[king]["E"]) & blockers # East
    #     s = (sB := ROOK_SQRS[king]["S"]) & blockers # South
    #     w = (wB := ROOK_SQRS[king]["W"]) & blockers # West
    #     ne = (neB := BISHOP_SQRS[king]["NE"]) & blockers # North East
    #     nw = (nwB := BISHOP_SQRS[king]["NW"]) & blockers # North West
    #     se = (seB := BISHOP_SQRS[king]["SE"]) & blockers # South East
    #     sw = (swB := BISHOP_SQRS[king]["SW"]) & blockers # South West

    #     dirs = [n, e, s, w, ne, nw, se, sw]
    #     bitboards = [nB, eB, sB, wB, neB, nwB, seB, swB]

    #     pins = {}
    #     for bb, func, pp, base_bb in zip(dirs, self.SCANS, self.POSSIBLE_PINNERS, bitboards): # iterate through all directions
    #         possible_pinner = None
    #         for sq in func(bb): # apply scan to get all the squares
    #             if self.piece_at(sq).colour == self.__turn: # same colour as the king, so it is a friendly piece
    #                 possible_pinner = sq
    #             if (p := self.piece_at(sq)).colour != self.__turn and p.piece_type in pp: # enemy piece that can pin
    #                 if possible_pinner is not None: # has seen a friendly piece before
    #                     pins[possible_pinner] = base_bb
    #                 break
    ...
    def legal_moves(self): # Legal moves
        ...
        for move in pseudo_legal_moves:
            # if move.from_square in pinned_pieces: # piece is pinned
            #     if bb_from_sqrs([move.to_square]) & pinned_pieces[move.from_square] != 0: # to square is on the same pin-line
