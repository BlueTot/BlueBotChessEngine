...
class Board:
    ...
    def generate_pawn_moves(self): # Generate pawn moves
        ...
        # Generate pawn move squares bitboard
        if self.__turn:
            single_pawn_sqrs = self.__board_bb[0] << 8 & self.__empty_squares()
            double_pawn_sqrs = single_pawn_sqrs << 8 & self.__empty_squares() & (BB_RANKS[2] | BB_RANKS[3])
        else:
            single_pawn_sqrs = self.__board_bb[1] >> 8 & self.__empty_squares()
            double_pawn_sqrs = single_pawn_sqrs >> 8 & self.__empty_squares() & (BB_RANKS[5] | BB_RANKS[4])

        # Single Push
        for sq in squares_from(single_pawn_sqrs):
            orig_sq = sq + (-8 if self.__turn else 8)
            if rank(sq) in ("1", "8"): # Promotion
        ...
        # Double push
        for sq in squares_from(double_pawn_sqrs):
            orig_sq = sq + (-16 if self.__turn else 16)
            moves.append(Move(orig_sq, sq))
    ...
    def attacks_mask(self, sq):
        ...
        if bb_sq & (self.__board_bb[2] | self.__board_bb[3]): # knights
            return KNIGHT_SQRS[sq]
        else:
            attacks = 0
            if bb_sq & (self.__board_bb[4] | self.__board_bb[5]) or bb_sq & (self.__board_bb[8] | self.__board_bb[9]): # bishops or queens
                attacks |= BB_DIAG_ATTACKS[sq][self.__blockers() & BB_DIAG_MASKS[sq]]
            if bb_sq & (self.__board_bb[6] | self.__board_bb[7]) or bb_sq & (self.__board_bb[8] | self.__board_bb[9]): # rooks or queens
                attacks |= BB_RF_ATTACKS[sq][self.__blockers() & BB_RF_MASKS[sq]]
            return attacks
        
    def pseudo_legal_piece_moves(self):
        moves = []
        non_pawns = self.__friendly_squares() & ~(self.__board_bb[0] | self.__board_bb[1])
        for from_square in squares_from(non_pawns):
            moves_bb = self.attacks_mask(from_square) & ~self.__friendly_squares()
            for to_square in squares_from(moves_bb):
    ...
    def pseudo_legal_moves(self): # Pseudo-legal moves
        ...
        return self.generate_pawn_moves() + self.pseudo_legal_piece_moves() + self.generate_castling_moves()
...
CURRENT RECORD FOR PERFT 5: 57.011s by v0.33 [2]

STARTING POSITION

v33 [2]

{'nodes': 20, 'captures': 0, 'checks': 0, 'en passant': 0, 'time': 0.006281}
{'nodes': 400, 'captures': 0, 'checks': 0, 'en passant': 0, 'time': 0.1461043}
{'nodes': 8902, 'captures': 34, 'checks': 12, 'en passant': 0, 'time': 0.7596839}
{'nodes': 197281, 'captures': 1576, 'checks': 469, 'en passant': 0, 'time': 3.3098203000000006}
{'nodes': 4865609, 'captures': 82719, 'checks': 27351, 'en passant': 258, 'time': 57.0116684}
done

v32
{'nodes': 20, 'captures': 0, 'checks': 0, 'en passant': 0, 'time': 0.0052597}
{'nodes': 400, 'captures': 0, 'checks': 0, 'en passant': 0, 'time': 0.1156553}
{'nodes': 8902, 'captures': 34, 'checks': 12, 'en passant': 0, 'time': 0.6483107}
{'nodes': 197281, 'captures': 1576, 'checks': 469, 'en passant': 0, 'time': 3.3240633999999996}
{'nodes': 4865609, 'captures': 82719, 'checks': 27351, 'en passant': 258, 'time': 59.7402299}
done
