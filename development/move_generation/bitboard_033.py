...
class Board:
    ...
    def __bishop_squares(self, sq, no_king=False, protected=False): # Get bishop squares
        return self.__intersection_sqrs(protected) & BB_DIAG_ATTACKS[sq][BB_DIAG_MASKS[sq] & self.__blockers(no_enemy_king=no_king)]
    ...
    def __rook_squares(self, sq, no_king=False, protected=False): # Get rook squares
        return self.__intersection_sqrs(protected) & BB_RF_ATTACKS[sq][BB_RF_MASKS[sq] & self.__blockers(no_enemy_king=no_king)]
    ...
    def generate_pawn_moves(self): # Generate pawn moves
        ...
        for sq in squares_from(single_pawn_sqrs):
            ...
            if rank(sq) in ("1", "8"): # Promotion
                for piece in (KNIGHT, BISHOP, ROOK, QUEEN):
                    yield Move(orig_sq, sq, promotion=piece)
            else:
                yield Move(orig_sq, sq)
        ...
        for Psq in squares_from(self.__board_bb[0 if self.__turn else 1]):
            for sq in squares_from(self.__pawn_atk_squares(Psq)):
                if rank(sq) in ("1", "8"): # Promotion
                    for piece in (KNIGHT, BISHOP, ROOK, QUEEN):
                        yield Move(Psq, sq, promotion=piece)
                else:
                    yield Move(Psq, sq)
        ...
        if epsq is not None:
            ...
            for psq in pattack_sqrs:
                if (p := self.piece_at(psq)) is not None and p.piece_type == PAWN and p.colour == self.__turn:
                    yield Move(psq, epsq, en_passant=True)
    ...
    def pseudo_legal_moves(self): # Pseudo-legal moves
        yield from self.generate_pawn_moves()
        yield from self.generate_knight_moves()
        yield from self.generate_bishop_moves()
        yield from self.generate_rook_moves()
        yield from self.generate_queen_moves()
    ...
    def legal_moves(self): # Legal moves
        for move in self.pseudo_legal_moves:
            ...
            if legal:
                yield move
        yield from self.generate_king_moves()
...
CURRENT RECORD FOR PERFT 5: 59.740s by Bitboard v0.32
