...
class Move:
    def __init__(self, from_square, to_square, en_passant=False, promotion=False, castling=False):
...
class Board:
    ...
    def generate_pawn_moves(self):
        ...
        if len(self.__move_stack) > 0:
            ...
            if self.__is_double_push(last_move):
                ...
                if right is not None and right.piece_type == PAWN and right.colour == self.__turn:
                    moves.append(Move(sq + 1, sq + 8 if self.__turn else sq - 8, en_passant=True))
    ...
    def push(self, move):
        ...
        if move.en_passant:
            self.__update_bb(Piece(PAWN, BLACK if self.__turn else WHITE), self.__remove_piece, s2 - 8 if self.__turn else s2 + 8)
